...
TEST(PENTAGON, GCenter)
{
    ...
    EXPECT_TRUE(doubleEqual(dist(B.getGCenter(), center), 0));
}
...
TEST(PENTAGON, operatorDouble_Aka_AREA)
{
    ...
    point center{12, 34}, dot{56, 78};
    Pentagon A(center, dot);
    double TrueArea =  5 * pow(dist(center, dot), 2) * sin((2*M_PI)/5) * 0.5;
    b0 = A.getDots()[0];
    b1 = A.getDots()[1];
    b2 = A.getDots()[2];
    b3 = A.getDots()[3];
    b4 = A.getDots()[4];
    Pentagon B({b0, b1, b2, b3, b4});
    EXPECT_TRUE(doubleEqual(TrueArea, double(B)));
}
...
TEST(RHOMBUS, operatorDouble_Aka_AREA)
{
    ...
    double TrueArea =  (dist(center, dot) * 2) * (dist(center, dot) * 2 * multi * 0.5);
    b0 = A.getDots()[0];
    b1 = A.getDots()[1];
    b2 = A.getDots()[2];
    b3 = A.getDots()[3];
...
TEST(HEXAGON, WrongDotsOrder)
{
    ...
    EXPECT_THROW(Hexagon A({b1, b0, b2, b3, b4}), std::underflow_error);
}
...
TEST(HEXAGON, operatorDouble_Aka_AREA)
{
    point b0, b1, b2, b3, b4, b5;
    point center{12, 34}, dot{56, 78};
    double TrueArea =  6 * pow(dist(center, dot), 2) * sin((2*M_PI)/6) * 0.5;
    Hexagon A(center, dot);
    b0 = A.getDots()[0];
    b1 = A.getDots()[1];
    b2 = A.getDots()[2];
...
TEST(HEXTAGON, operatorEqualFalse2)
{
    point b0, b1, b2, b3, b4, b5;
    point c0{1,2};
    point center1{0, 0}, dot1{1, 0};
    Hexagon A1(center1, dot1);
    Pentagon C(center1, c0);
    b0 = A1.getDots()[0];
    b1 = A1.getDots()[1];
    b2 = A1.getDots()[2];
    b3 = A1.getDots()[3];
...
TEST(FCOLLECTION, GetAllArea)
{
    ...
    F.pushBack(&A2);
    F.pushBack(&A3);
    EXPECT_TRUE(doubleEqual(F.getAllArea(), double(A1) + double(A2) + double(A3)));
}
